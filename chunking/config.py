...
CHUNKING_CONNECTION_TIMEOUT = int(os.getenv("CHUNKING_CONNECTION_TIMEOUT", "1800"))
...
# Vision LLM backend: ollama | vllm (falls back to chunking, then global)
VISION_LLM_BACKEND = os.getenv("VISION_LLM_BACKEND", os.getenv("CHUNKING_LLM_BACKEND", os.getenv("LLM_BACKEND", "ollama")))
...
# VLLM URL for vision processing (falls back to chunking, then global VLLM_URL)
VISION_VLLM_URL = os.getenv("VISION_VLLM_URL", os.getenv("CHUNKING_VLLM_URL", os.getenv("VLLM_URL", "http://localhost:8000")))
...
VISION_MAX_CONCURRENT = int(os.getenv("VISION_MAX_CONCURRENT", "2"))

# Temperature for vision model
VISION_TEMPERATURE = float(os.getenv("VISION_TEMPERATURE", "0.3"))

# Max tokens for vision model output
# 500 is safe for scanned PDFs with dense content; lower to 200 for simple images
